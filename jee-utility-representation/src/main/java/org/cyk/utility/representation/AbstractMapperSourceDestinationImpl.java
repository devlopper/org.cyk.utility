...
public abstract class AbstractMapperSourceDestinationImpl<SOURCE,DESTINATION> extends org.cyk.utility.__kernel__.mapping.MapperSourceDestination.AbstractImpl<SOURCE,DESTINATION> {
    ...
    public void __listenPostConstruct__() {
		if(__destinationClass__ == null) {
			String destinationClassName = ClassHelper.buildName(getClass().getPackageName(), getClass().getSimpleName()
					, new NamingModel().server().representation().entities().setSuffix("DtoMapperImpl"), new NamingModel().server().persistence().entities());
			__destinationClass__ = ValueHelper.returnOrThrowIfBlank("persistence entity class",(Class<DESTINATION>) ClassHelper.getByName(destinationClassName));
		}
