...
public class LayoutUnitTest extends AbstractWeldUnitTest {
	...
	public void buildTwoColumns_ui_g_numberOfRowsAndNumberOfColumnsDeduced() {
		Layout layoutTwoColumns = Builder.build(Layout.class,Map.of("identifier","layout two columns",Layout.FIELD_CELL_WIDTH_UNIT,Cell.WidthUnit.UI_G
	...
	public void buildTwoColumns_flex_numberOfRowsAndNumberOfColumnsDeduced() {
		Layout layoutTwoColumns = Builder.build(Layout.class,Map.of("identifier","layout two columns",Layout.FIELD_CELL_WIDTH_UNIT,Cell.WidthUnit.FLEX
				,"cells",CollectionHelper.listOf(
					Cell.build(Map.of(Cell.FIELD_IDENTIFIER,"first name",Cell.FIELD_WIDTH,6,Cell.FIELD_WIDTH_UNIT,Cell.WidthUnit.FLEX))
					,Cell.build(Map.of(Cell.FIELD_IDENTIFIER,"last names",Cell.FIELD_WIDTH,6,Cell.FIELD_WIDTH_UNIT,Cell.WidthUnit.FLEX))
					)
				));
		assertThat(layoutTwoColumns.getNumberOfRows()).isEqualTo(1);
		assertThat(layoutTwoColumns.getNumberOfColumns()).isEqualTo(2);
		...
		assertThat(CollectionHelper.getElementAt(layoutTwoColumns.getCells(),0)).isNotNull();
		assertThat(CollectionHelper.getElementAt(layoutTwoColumns.getCells(),0).getStyleClass()).isNotEmpty();
		assertThat(CollectionHelper.getElementAt(layoutTwoColumns.getCells(),0).getStyleClass()).contains("p-col-6");
		assertThat(CollectionHelper.getElementAt(layoutTwoColumns.getCells(),1).getStyleClass()).contains("p-col-6");
	}
}
