...
public class OrderStringBuilderUnitTest extends AbstractWeldUnitTest {
	...
	public void order_map(){
		assertThat(OrderStringBuilder.getInstance().build(Map.of("t.f",SortOrder.DESCENDING))).isEqualTo("ORDER BY t.f DESC");
		assertThat(OrderStringBuilder.getInstance().build(Map.of("t.f",SortOrder.DESCENDING,"t.f2",SortOrder.ASCENDING))).isEqualTo("ORDER BY t.f DESC,t.f2 ASC");
	}
}