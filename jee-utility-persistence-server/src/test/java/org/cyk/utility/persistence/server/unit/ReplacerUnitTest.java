...
public class ReplacerUnitTest extends AbstractUnitTest {
	...
	public void orderBy_code_desc_clause_namedQuery(){
		QueryManager.getInstance().setIsRegisterableToEntityManagerFactory(Boolean.FALSE);
		QueryManager.getInstance().register(new Query().setIdentifier("q1").setValue("SELECT t FROM Tuple t ORDER BY t.code ASC"));
