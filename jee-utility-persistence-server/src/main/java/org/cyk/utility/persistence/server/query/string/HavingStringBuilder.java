...
public interface HavingStringBuilder {
	...
	public abstract class AbstractImpl extends AbstractObject implements HavingStringBuilder,Serializable {
		private static final long serialVersionUID = 1L;		
		
		@Override
		public String build(Predicate predicate) {
			if(predicate == null)
				throw new RuntimeException("Having predicate is required");
		...
		public String build(Collection<String> predicates) {
			if(CollectionHelper.isEmpty(predicates))
				throw new RuntimeException("Having predicates are required");
	...
	static HavingStringBuilder getInstance() {
		return Helper.getInstance(HavingStringBuilder.class, INSTANCE);
	}
	
	Value INSTANCE = new Value();
	String FORMAT = "HAVING %s";
	String AND = "AND";
