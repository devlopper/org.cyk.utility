...
public class CascadeStyleSheetHelperUnitTestBenchmark extends AbstractWeldUnitTestBenchmark {
	...
	protected void __listenBefore__() {
		...
		__inject__(FunctionRunnableMap.class).set(StyleClassBuilderWidthImpl.class, StyleClassBuilderWidthCssFunctionRunnableImpl.class);
	}
	
	@Test
	public void buildFieldName(){
		execute(new Jobs().setName("Build style class proportion").setNumberOfRound(1000000)
			.add("StyleClassBuilderWidth",new Runnable() {
			@Override
			public void run() {
				__inject__(StyleClassBuilderWidth.class).setDevice(null).setWidth(null).execute();
			}
		}).add("CascadeStyleSheetHelper.buildStyleClassProportion", new Runnable() {
			@Override
			public void run() {
				__inject__(CascadeStyleSheetHelper.class).buildStyleClassProportion(null, null);
			}
		})
			);
	}
	
	public static class StyleClassBuilderWidthCssFunctionRunnableImpl extends AbstractFunctionRunnableImpl<StyleClassBuilderWidth> implements Serializable {
		private static final long serialVersionUID = 1L;
