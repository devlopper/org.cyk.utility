...
public class CollectionDataTableSheetPage extends AbstractPageContainerManagedImpl implements Serializable {
	...
	protected void __listenPostConstruct__() {
		...
		dataTable.addRecordMenuItemByArguments(MenuItem.FIELD_VALUE,"Edit",MenuItem.FIELD_ICON,"fa fa-pencil",MenuItem.FIELD_LISTENER,new AbstractAction.Listener.AbstractImpl() {			
			@Override public void run(AbstractAction action) {
				...
				p.getQuery().setValue("entityidentifier="+((Namable)action.get__argument__()).getIdentifier());
				try {
		...
		dataTable.addRecordMenuItemByArguments(MenuItem.FIELD_VALUE,"Delete",MenuItem.FIELD_ICON,"fa fa-remove",MenuItem.FIELD_LISTENER,new AbstractAction.Listener.AbstractImpl() {			
			@Override public void run(AbstractAction action) {__inject__(NamableController.class).delete((Namable) action.get__argument__());}
		...
		dataTable.addHeaderToolbarLeftCommands(
				Builder.build(CommandButton.class,Map.of(CommandButton.FIELD_VALUE,"Opération 1. Min1",CommandButton.FIELD___COLLECTION__,dataTable
					...
					protected void runShowDialogWhenShowable(AbstractAction action) {
						...
						super.runShowDialogWhenShowable(action);
					}
				}.setCollection(dataTable),CommandButton.ConfiguratorImpl.FIELD_COLLECTION_UPDATABLE,Boolean.FALSE))
				,Builder.build(CommandButton.class,Map.of(CommandButton.FIELD_VALUE,"Opération 2. Min2",CommandButton.FIELD___COLLECTION__,dataTable
						,CommandButton.FIELD_LISTENER,new AbstractAction.Listener.AbstractImpl() {
					@Override
					protected void runShowDialogWhenShowable(AbstractAction action) {
				...
				,Builder.build(CommandButton.class,Map.of(CommandButton.FIELD_VALUE,"Opération 3. No Constraint",CommandButton.FIELD___COLLECTION__,dataTable
						,CommandButton.FIELD_LISTENER,new AbstractAction.Listener.AbstractImpl() {
					@Override
					protected void runShowDialogWhenShowable(AbstractAction action) {
